...
# Đọc file .tsv/.csv với pandas, kiểm soát lỗi, dùng chuẩn utf-8.
def read_table(path):
...
def load_data_record(data_root, split, language_list, subsample_rate=1,
    ...
    for lang in language_list:
        ...
        data_pair = read_table(os.path.join(data_root, 
                                            f"covost_v2.{data_lang_code}_en.{split}.tsv"))
        ...
        if split == 'train' and expanded_language_list is not None and lang in expanded_language_list:
            ...
            test_data_pair = read_table(os.path.join(data_root, f"covost_v2.{data_lang_code}_en.test.tsv"))
            dev_data_pair = read_table(os.path.join(data_root, f"covost_v2.{data_lang_code}_en.dev.tsv"))
            ...
            expanded_data_pair['audio_root'] = os.path.join(expanded_data_root, 'clips')
